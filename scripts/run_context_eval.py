...
from app.services.context.budget import ContextBudgetExceeded, canonical_json, estimate_tokens
...
def _snapshot(pending_id: int = 42) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        recent_experiments=[
            {"id": index, "status": "complete", "summary": "growth observation " * 8}
...
def _default_cases() -> list[dict[str, Any]]:
    cases = []
    for turn_count in (10, 40, 100, 200):
        history = [
            {
        ...
        cases.extend(
            [
                {
                    "id": f"workflow_{turn_count}_turns",
                ...
                {
                    "id": f"knowledge_{turn_count}_turns",
...
def run_context_eval(cases: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for case in cases or _default_cases():
        ...
        variant_latency_ms: dict[str, float] = {}
        variant_budget_exceeded: dict[str, bool] = {}
        full = None
        for name, features in variants.items():
            ...
            try:
                ...
                variant_budget_exceeded[name] = False
            except ContextBudgetExceeded as exc:
                envelope = None
                variant_tokens[name] = exc.report.estimated_input_tokens
                variant_budget_exceeded[name] = True
            variant_latency_ms[name] = (time.perf_counter() - started) * 1000.0
        ...
        results.append(
            {
                ...
                "prefix_stable_across_dynamic_change": (
                    full is not None
                    and changed is not None
                    and full.budget_report.prefix_hash == changed.budget_report.prefix_hash
                ),
                "compression_record_count": len(full.compression_records) if full else 0,
                "active_skill_count": len(full.active_skills) if full else 0,
                "protected_pending_present": (
                    full is not None
                    and (
                        "42" in canonical_json(full.dynamic_context)
                        or "42" in full.status_bar.render()
                    )
                ),
            }
        )
    ...
    return {
        "status": (
            "passed"
            if all(
                item["prefix_stable_across_dynamic_change"]
                and item["protected_pending_present"]
                for item in results
            )
            else "failed"
        ),
...
def main() -> None:
    ...
    print(rendered)
    raise SystemExit(
        0 if report["status"] == "passed" and report["meets_25pct_reduction_target"] else 1
    )
