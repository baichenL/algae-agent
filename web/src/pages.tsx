...
export function TestLabPage() {
  ...
  const mutation = useMutation({ mutationFn: ({ id, parameters }: { id: string; parameters: Record<string, unknown> }) => api.startScenario(id, parameters), onSuccess: result => navigate(`/runs/${encodeURIComponent(result.run_id)}`) })
